...
def loginInicial(request):
    ...
    projects = Project.objects.exclude(personvote__person_id=request.user.pk, personvote__etapa__contains=ETAPA).filter(ativo='SIM')
    return render(request, 'votaProject.html', {'projects':projects, 'title':'VOTE'})

@login_required(login_url='/persons/login/')
def loginVoted(request):
    #projects = Project.objects.extra("select p.* from projects_project p inner join persons_personvote pv on (pv.project_id = p.id and pv.etapa like '%%%s%%' and pv.person_id = %d) where p.ativo = 'SIM' order by p.area, p.tema"%(ETAPA,request.user.pk))
    projects = Project.objects.filter(personvote__person_id=request.user.pk, personvote__etapa__contains=ETAPA, ativo='SIM')
...
def ranking(request):
    ...
    #PersonVote.objects.filter(project__area='INFORMÁTICA', project__ativo='SIM', etapa__icontains=ETAPA)[:15]
